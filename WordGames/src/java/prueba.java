    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       response.setContentType("text/html;charset=UTF-8");
        try (PrintWriter out = response.getWriter()) {
            ...
            cliente.setNombre("Administrador");
            cliente.setApellidos("");
            cliente.setNombreusu("root");
            cliente.setContraseña("root");
            cliente.setCiudad("Sevilla");
            cliente.setCodigoPostal(12345);
            
            
            String mensaje = "Se ha creado el Administrador";
            ClienteJpaController ejc = 
                    new ClienteJpaController(Persistence.createEntityManagerFactory("WordgamesPU"));
            try {
                ejc.create(cliente);
            } catch (Exception ex) {
